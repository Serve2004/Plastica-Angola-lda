...
export function Header() {
  ...
  return (
    <header
            className={`fixed top-0 left-0 right-0 px-2 sm:px-4 md:px-8 xl:px-10 py-8 sm:py-10 md:py-2.25 flex items-center justify-between z-100 overflow-hidden h-14 sm:h-16 transition-all duration-1000  ${bgWhite ? "bg-white" : "bg-transparent"}`}
          >
            ...
            <nav className="flex items-center gap-2 sm:gap-4">
              ...
              >
                {!menuOpen ? <ListIcon size={24} /> : <XIcon size={24} />}
              </button>
            </nav>
            {menuOpen && (
              <div
                className={`fixed top-14 sm:top-16 left-0 w-full bg-white shadow-md z-50 md:hidden transform transition-all duration-500 ${
                  menuOpen
