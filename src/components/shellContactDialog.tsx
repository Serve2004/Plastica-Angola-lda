...
export function ShellContactDialog({ open, onClose }: ContactDialogProps) {
  ...
  if (success) {
    return <StepSuccess onClose={handleClose} type="primary" />;
  }
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <div
          className="relative bg-[#EFEFEF] rounded-2xl shadow-2xl w-full max-w-sm flex flex-col overflow-hidden"
          onClick={(e) => e.stopPropagation()}
        >
          <button onClick={handleClose} className="absolute top-4 right-4 w-7 h-7 flex items-center justify-center rounded-full hover:bg-zinc-200 text-zinc-400 hover:text-zinc-700 transition-colors" aria-label="Fechar">
            <X size={15} />
          </button>
          ...
          <div className="px-6 pt-6 pb-5">
            <div className="flex items-center gap-2.5 mb-1">
              <div className="w-9 h-9 rounded-[8px] bg-[#FF6400] flex items-center justify-center shrink-0">
                <img src={assets.social} alt="Social Media" className="w-4 sm:w-6 h-4 sm:h-6" />
              </div>
              <h2 className="text-[24px] font-bold text-zinc-900 leading-tight">Fale Connosco</h2>
            </div>
          ...
          <div className="flex flex-col gap-4 px-6 pb-4">
            <Field label="Nome completo">
              <input type="text" placeholder="O seu nome" value={form.name} onChange={(e) => handleChange("name", e.target.value)} className={inputClass} />
            </Field>
            <Field label="E-mail">
              <input type="email" placeholder="nome@exemplo.com" value={form.email} onChange={(e) => handleChange("email", e.target.value)} className={inputClass} />
            </Field>
            <Field label="Telefone">
              <input type="tel" placeholder="+244" value={form.phone} onChange={(e) => handleChange("phone", e.target.value)} className={inputClass} />
            </Field>
            <Field label="Mensagem">
              <textarea placeholder="Conte-nos como podemos ajudar..." value={form.message} onChange={(e) => handleChange("message", e.target.value)} rows={4} className={`${inputClass} resize-none`} />
            </Field>
          </div>
